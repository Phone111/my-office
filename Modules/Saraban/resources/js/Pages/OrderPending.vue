...
<template>
    <Head title="แฟ้มรอแนบไฟล์คำสั่ง" />

    <AuthenticatedLayout>
        <template #header>
            <h2 class="text-xl font-semibold leading-tight text-gray-800">แฟ้มรอแนบไฟล์คำสั่ง</h2>
        </template>
        ...
        <div class="py-12">
            <div class="mx-auto max-w-5xl space-y-6 px-4 sm:px-6 lg:px-8">
                ...
                <EmptyState v-if="rows.length === 0" title="ไม่มีคำสั่งที่รอแนบไฟล์" />
                ...
                <div v-else class="overflow-hidden rounded-2xl bg-white shadow-sm ring-1 ring-gray-100">
                    <div class="flex items-center gap-2 border-b border-gray-100 bg-indigo-50/60 px-6 py-3">
                        ...
                        <span class="font-semibold text-gray-800">แฟ้มรอแนบไฟล์คำสั่ง</span>
                    ...
                    <table class="min-w-full divide-y divide-gray-100">
                        <thead class="bg-gray-50">
                            <tr class="text-left text-xs font-medium uppercase tracking-wider text-gray-500">
                                <th class="px-6 py-3 text-center">คำสั่งที่</th>
                                <th class="px-6 py-3">เรื่อง</th>
                                <th class="px-6 py-3">ตั้งแต่วันที่</th>
                                <th class="px-6 py-3">สั่ง ณ วันที่</th>
                        ...
                        <tbody class="divide-y divide-gray-50">
                            <tr v-for="r in rows" :key="r.id" class="text-sm text-gray-700 hover:bg-gray-50">
                                ...
                                <td class="px-6 py-4 text-gray-500">{{ r.effective_thai ?? '—' }}</td>
                                <td class="px-6 py-4 text-gray-500">{{ r.order_thai ?? '—' }}</td>
                                <td class="px-6 py-4 text-right">
                                    <Link :href="route('saraban.orders.attach-form', r.id)" class="inline-flex items-center gap-1.5 font-medium text-indigo-600 hover:text-indigo-800">แนบไฟล์ →</Link>
                                </td>
                            </tr>
                        </tbody>
                    </table>
