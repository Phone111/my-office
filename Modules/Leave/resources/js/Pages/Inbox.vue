...
<template>
    <Head title="ตรวจสอบวันลา" />

    <AuthenticatedLayout>
        <template #header>
            <h2 class="text-xl font-semibold leading-tight text-gray-800">ตรวจสอบวันลา</h2>
        </template>
        ...
        <div class="py-12">
            <div class="mx-auto max-w-5xl space-y-6 px-4 sm:px-6 lg:px-8">
                ...
                <div v-if="requests.length === 0" class="rounded-2xl bg-white shadow-sm ring-1 ring-gray-100">
                    <EmptyState title="ไม่มีใบลารอการดำเนินการจากคุณ" />
                </div>
                ...
                <div v-else class="overflow-hidden rounded-2xl bg-white shadow-sm ring-1 ring-gray-100">
                    <div class="flex items-center gap-2 border-b border-gray-100 bg-indigo-50/60 px-6 py-3">
                        ...
                        <span class="font-semibold text-gray-800">แฟ้มตรวจสอบวันลา</span>
                    ...
                    <table class="min-w-full divide-y divide-gray-100">
                        ...
                        <tbody class="divide-y divide-gray-50">
                            <tr v-for="r in requests" :key="r.id" class="text-sm text-gray-700 hover:bg-gray-50">
                                <td class="px-6 py-4 font-medium text-gray-900">{{ r.subject }}</td>
                                <td class="px-6 py-4 text-gray-500">{{ r.sender }}</td>
                                <td class="px-6 py-4 text-gray-500">{{ r.fiscal_year }}</td>
                                <td class="px-6 py-4 text-gray-500">{{ r.sent_thai }}</td>
                                <td class="px-6 py-4 text-right">
                                    <Link :href="route('leave.requests.proposal', r.id)" class="font-medium text-indigo-600 hover:text-indigo-800">ดำเนินการต่อ →</Link>
                                </td>
                            </tr>
                        </tbody>
                    </table>
